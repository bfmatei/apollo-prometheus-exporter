...
function generateHooks(metrics) {
  ...
  return {
    ...
    async requestDidStart(requestContext) {
      ...
      console.log('metrics label', getLabelsFromContext(requestContext));
      actionMetric(
        { name: metrics_1.MetricsNames.QUERY_STARTED, labels: getLabelsFromContext(requestContext) },
        requestContext
      );
      return {
        async parsingDidStart(context) {
          actionMetric(
            { name: metrics_1.MetricsNames.QUERY_PARSE_STARTED, labels: getLabelsFromContext(context) },
            context
          );
          return async (err) => {
            if (err) {
              actionMetric(
                { name: metrics_1.MetricsNames.QUERY_PARSE_FAILED, labels: getLabelsFromContext(context) },
                context
              );
            }
          };
        ...
        async validationDidStart(context) {
          actionMetric(
            { name: metrics_1.MetricsNames.QUERY_VALIDATION_STARTED, labels: getLabelsFromContext(context) },
            context
          );
          return async (err) => {
            if (err) {
              actionMetric(
                { name: metrics_1.MetricsNames.QUERY_VALIDATION_FAILED, labels: getLabelsFromContext(context) },
                context
              );
            }
          };
        ...
        async executionDidStart(context) {
          actionMetric(
            { name: metrics_1.MetricsNames.QUERY_EXECUTION_STARTED, labels: getLabelsFromContext(context) },
            context
          );
          return {
            ...
            async executionDidEnd(err) {
              if (err) {
                actionMetric(
                  { name: metrics_1.MetricsNames.QUERY_EXECUTION_FAILED, labels: getLabelsFromContext(context) },
                  context
                );
              }
            }
